...
func! s:ctrl_s(cnt, here) abort
  ...
  elseif bufexists(b)
    ...
    let @# = origbuf
    tnoremap <silent> <buffer> <C-s> <C-\><C-n>:silent call <SID>ctrl_s(0, v:false)<CR>
  endif
  let g:term_shell.prevwid = curwinid
  setlocal nobuflisted
endfunc

nnoremap <silent> <C-s> :<C-u>silent call <SID>ctrl_s(v:count, v:false)<CR>
nnoremap <silent> '<C-s> :<C-u>silent call <SID>ctrl_s(v:count, v:true)<CR>
]])
