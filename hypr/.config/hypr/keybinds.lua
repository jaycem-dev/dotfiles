...
local fileManager = { cmd = "kitty -1 --app-id yazi yazi", class = "yazi" }
local menu = "fuzzel"
local browser = { cmd = "brave", class = "brave-browser" }

-- launch or focus by class
local function spawn_or_focus(app)
	local class = app.class or app.cmd -- if class is not defined, use cmd
	return function()
...
hl.bind(mod .. " + E", spawn_or_focus(fileManager))
...
hl.bind("XF86AudioRaiseVolume", hl.dsp.exec_cmd("~/.local/bin/volume up"), { locked = true, repeating = true })
hl.bind("XF86AudioLowerVolume", hl.dsp.exec_cmd("~/.local/bin/volume down"), { locked = true, repeating = true })
hl.bind(
	"XF86AudioMute",
